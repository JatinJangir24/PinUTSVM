...
%
% Predict_Y = pinGTSVM(TestData,TrainData,FunPara)
%
% Input:
%    TestData - Test Data Matrix. Each row vector of fea is a data point.
%
% Reference:
...
tic;
[no_input, no_col] = size(TrainData);
obs = TrainData(:, no_col);
A = TrainData(obs == 1, 1:end - 1);
B = TrainData(obs ~= 1, 1:end - 1);
...
m1 = size(A, 1);
m2 = size(B, 1);
m3 = size(UniversumData, 1);
m = m1 + m2;
um = size(UniversumData, 1);
e1 = ones(m1, 1);
e2 = ones(m2, 1);
eu = ones(um, 1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if strcmp(kerfPara.type, 'lin')
    ...
    %         X3 = [kernelfun(UniversumData, kerfPara, X), e2];
...
if strcmp(kerfPara.type,'lin')
    ...
    C=[A;B];
    % H=kernelfun(TestData,kerfPara,C);
    %%%%%%%%%%%%%%%%%%%%%%%5
    mu = kerfPara.pars(1);
    m1 = size(TestData,1);
    K = zeros(m1, m);
